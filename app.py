...
def venues():
  ...
  allVenues=Venue.query.order_by(Venue.city, Venue.state, Venue.name).all()

  lastCity = { "city": "", "state": "", "venues": []}
  data = []
  for venue in allVenues:
    if (lastCity["city"] != venue.city and lastCity["state"] != venue.state):
      lastCity = { "city": venue.city, "state": venue.state, "venues": []}
      data.append(lastCity)

    lastCity["venues"].append({
      "id": venue.id,
      "name": venue.name,
...
def show_artist(artist_id):
  ...
  genres = artist.genres if artist.genres is not None else ''
  data={
    "id": artist_id,
    "name": artist.name,
    "genres": genres.split(','),
